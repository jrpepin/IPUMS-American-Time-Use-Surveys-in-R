...
### Create summary variables of each activity duration
```{r results = 'hide'}
...
```
```{r echo = FALSE}
view <- select(data, caseid, leisure, sleep, hswrk, ccare, work, socl, actl, pass, tele)
knitr::kable(
  head(view, n = 5), 
...
```{r echo = FALSE}
view <- select(max, caseid, tele, socl, actl, pass, leisure, sleep, hswrk, ccare, work)
head(view, n = 5)
remove(view)
```
...
atus <- select(atus, caseid, wt06, year, tele, socl, actl, pass, leisure, sleep, hswrk, ccare, work, socl, actl, pass, tele, telesolo, telesp, age, sex, mar, exfam, kidu2, hh_numownkids, employ, educ, raceth, weekend, region)
# Listwise deletion
atus <- na.omit(atus)
```
