...
rec2 <- data %>% 
  ...
  select(caseid, age, sex, race, hispan, marst, educ, fullpart, spousepres, spsex,  
         hh_numownkids, kidund13, day, wt06, wt20)
...
atus <- atus %>%
  mutate(
    partnered = case_when(
      mar == "Married" | mar == "Cohabiting"          ~ "Partnered",
      mar == "Single"  | mar == "Divorced\nSeparated" ~ "Single",
      TRUE ~ NA_character_
    ))
...
# Listwise deletion
atus <- select(atus, caseid, wt20, year, ccare, hswrk, leisure, sleep, socl, actl, pass, tele, age, sex, 
               mar, partnered, exfam, kidu2, hh_numownkids, employ, educ, raceth, weekend, region, ownrent)
atus <- na.omit(atus) 
...
#Housework
lm_hswrk <- lm(hswrk  ~ mar + exfam + hh_numownkids + kidu2 + educ + employ + raceth + age + weekend + region + ownrent, data = atus, weight=wt20)
phswrk   <- ggeffect(lm_hswrk, terms = "mar")

#Leisure
lm_leis <- lm(leisure ~ mar + exfam + hh_numownkids + kidu2 + educ + employ + raceth + age + weekend + region + ownrent, data = atus, weight=wt20)
pleis   <- ggeffect(lm_leis, terms = "mar")

#Childcare
lm_care <- lm(ccare   ~ mar + exfam + hh_numownkids + kidu2 + educ + employ + raceth + age + weekend + region + ownrent, data = atus, weight=wt20)
pcare   <- ggeffect(lm_care, terms = "mar")

#Sleep
lm_sleep <- lm(sleep  ~ mar + exfam + hh_numownkids + kidu2 + educ + employ + raceth + age + weekend + region + ownrent, data = atus, weight=wt20)
...
#Housework
lm_hswrk <- lm(hswrk  ~ partnered + exfam + hh_numownkids + kidu2 + educ + employ + raceth + age + weekend + region + ownrent, data = atus, weight=wt20)
phswrk   <- ggeffect(lm_hswrk, terms = "partnered")

#Leisure
lm_leis <- lm(leisure ~ partnered + exfam + hh_numownkids + kidu2 + educ + employ + raceth + age + weekend + region + ownrent, data = atus, weight=wt20)
pleis   <- ggeffect(lm_leis, terms = "partnered")

#Childcare
lm_care <- lm(ccare   ~ partnered + exfam + hh_numownkids + kidu2 + educ + employ + raceth + age + weekend + region + ownrent, data = atus, weight=wt20)
pcare   <- ggeffect(lm_care, terms = "partnered")

#Sleep
lm_sleep <- lm(sleep  ~ partnered + exfam + hh_numownkids + kidu2 + educ + employ + raceth + age + weekend + region + ownrent, data = atus, weight=wt20)
psleep   <- ggeffect(lm_sleep, terms = "partnered")
...
pred$x     <- ordered(pred$x, levels = c("Partnered", "Single"))
pred$group <- ordered(pred$group, levels = c("Childcare", "Housework", "Leisure", "Sleep"))

# Calculated difference minutes
pred <- pred %>%
  group_by(group) %>%
  mutate(diff = predicted - predicted[x == "Partnered"])
...
pred %>%
  ...
  geom_col() +
  facet_grid(~group) +
  ggtitle("Single Mothers Report Mor Childcare and Partnered Mothers Report More Housework") +
  labs(x = NULL, y = NULL, subtitle = "Predicted minutes per day with model controls",
       caption = "Source: American Time Use Surveys (2020) \n Models control for extra adults, number of household kids, kids under 2, 
       education, employment, race-ethnicity, age, weekend diary day, home ownership and region") +
  theme_minimal() +
  theme(plot.subtitle = element_text(vjust = 1),
        ...
        panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank()) +
  scale_y_continuous(breaks = c(-30, -20, -10, 0, 10, 20, 30), labels=c("-30", "-20", "-10", "Partnered mothers' \n minutes per day", "10", "20", "30")) +
  scale_fill_manual(values=c("#18BC9C", "#F39C12", "#E74C3C")) +
  geom_errorbar(aes(ymin=diff-std.error, ymax=diff+std.error), width=.2,
                position=position_dodge(.9), color="grey")